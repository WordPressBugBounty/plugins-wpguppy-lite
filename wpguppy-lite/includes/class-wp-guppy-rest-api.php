...
if (!class_exists('WP_GUPPY_LITE_RESTAPI')) {
	...
	class WP_GUPPY_LITE_RESTAPI  extends WP_REST_Controller
	{
		 ...
		 *
		 * Authenticates the user by validating the provided JWT token.
		 * 
		...
		public function getProfileInfo($data)
		{
			...
			if (!empty($fetchResults)) {
				...
				if (!empty($info['user_image'])) {
					$userImage 				= unserialize($info['user_image'], ['allowed_classes' => false]);
		...
		{
			$params     = ! empty($data->get_params())	? $data->get_params()	: '';
			$json = $filterData = $unReadContent =  array();
			$requestCount 	= $onetoOneChatCount = 0;
			$loginedUser 	= $this->userId;
			$userType 		= isset($params['userType']) 	? intval($params['userType']) : 1;
		...
		public function updateProfileInfo($data)
		{
			...
			$userName 		= !empty($params['userName']) 	? sanitize_text_field($params['userName']) : '';
			$userEmail 		= !empty($params['userEmail']) 	? sanitize_email($params['userEmail']) : '';
		...
		public function updateGuppyMessage($data)
		{
			...
			$type 		= 'error';
			$chatId 		= !empty($params['chatId']) 	? sanitize_text_field($params['chatId']) : '';
			$receiverId 	= $this->userId;
			$chatType 		= !empty($params['chatType']) 	? intval($params['chatType']) 	: 0;
			$messageCounter = 0;

			if (!empty($chatId) && !$this->validateChatId($chatId)) {
				$json['type'] = 'error';
				$json['message_desc']   = esc_html__('Invalid chat id!', 'wp-guppy');
				return new WP_REST_Response($json, 203);
			}


			$chatkey 		= explode('_', $chatId);
			$chatkey[0] 	= isset($chatkey[0]) ? esc_sql($chatkey[0]) : 0;
			$chatkey[1] 	= isset($chatkey[1]) ? esc_sql($chatkey[1]) : 0;
		...
		public function getUserMessageslist($data)
		{
			...
			$searchQuery 	= !empty($params['search']) ? esc_sql(sanitize_text_field($params['search'])) : '';
			$loginedUser 	= $this->userId;
			$chatType 		= !empty($params['chatType']) ? intval($params['chatType']) : '1';
			...
			if (!empty($fetchResults)) {
				foreach ($fetchResults as $result) {
					...
					if ($message != '') {
						if ($messageType == 4) {
							$message = is_serialized($message) ? unserialize($message, ['allowed_classes' => false]) : $message;
						} else {
							$message = html_entity_decode(stripslashes($message), ENT_QUOTES);
						}
					}
		...
		{
			$params     	= ! empty($data->get_params()) 		? $data->get_params() 		: '';
			$chatMessages  	= $json = $filterData = array();
			$offset 		= !empty($params['offset']) 	? intval($params['offset']) 	: 0;
			$receiverId 	= isset($params['receiverId']) ? sanitize_text_field($params['receiverId']) : 0;
			$chatType 		= !empty($params['chatType']) 	? intval($params['chatType']) 	: 0;
			$loginedUser 	= $this->userId;
			$userType 		= isset($params['userType']) 	? intval($params['userType']) 			: 1;
			$filterData = array();
			...
			$filterData['chatType'] 	= $chatType;
			$filterData['userId'] 		= $receiverId;
			if (!empty($receiverId) && !$this->isNumericOrAlphaNumeric($receiverId)) {
				$json['type'] 			= 'error';
				$json['message_desc']   = esc_html__('Invalid receiver ID!', 'wp-guppy');
				return new WP_REST_Response($json, 203);
			}
			...
			if (!empty($fetchResults)) {
				...
				foreach ($fetchResults as $result) {
					...
					if (
						($result['chat_type'] == 3 && $result['sp_sender_id'] != $loginedUser)
						|| $result['sender_id'] != $loginedUser
					) {
						$isSender = false;
					}
					...
					if ($result['message_type'] == '4') {
						$message = is_serialized($result['message']) ? unserialize($result['message'], ['allowed_classes' => false]) : $result['message'];
					} else {
					...
					$messageData['replyMessage'] 		= !empty($result['reply_message']) ? unserialize($result['reply_message'], ['allowed_classes' => false]) : NULL;
		...
		public function getGuppyChatAction($filterData)
		{
			...
			if (!empty($chatActions)) {
				if ($filterData['actionType'] == '0') {
					...
					is_array($filterData['actionType'])
		 ...
		 * @since    1.0.0
		 */
		public function isNumericOrAlphaNumeric($value)
		{
		...
		{
			$params     	= !empty($data->get_params()) 		? $data->get_params() 		: '';
			$messageData 	= $attachmentData = $replyMessage = $json  = array();
			$verifyMember 	= false;
			$receiverId 		= !empty($params['receiverId']) 	? sanitize_text_field($params['receiverId']) 	: 0;
			$chatType 			= !empty($params['chatType']) 		? intval($params['chatType']) 		: 0;
			$messageType 		= !empty($params['messageType']) 	? intval($params['messageType']) 	: 0;
			$message 			= !empty($params['message']) 		? esc_sql(sanitize_text_field($params['message'])) : '';
			...
			if (!empty($replyId)) {
				...
				if (!empty($messageDetail)) {
					...
					$replyMessage['attachmentsData'] = !empty($messageDetail['attachments']) ? unserialize($messageDetail['attachments'], ['allowed_classes' => false]) : '';
				}
			}
			...
			if ($response) {
				...
				$messagelistData = $filterData = $chatNotify = array();
		 ...
		 * @since    1.0.0
		 */
		public function validateChatId($chatId)
		{
			return preg_match('/^(?:[a-z0-9]+(?:_[a-z0-9]+){1,2}|[0-9]+|[a-z0-9]+)$/i', $chatId);
		}
		...
		public function messengerChatInfo($data)
		{
			...
			$chatInfo 	= $json  = array();
			$loginedUser 	= $this->userId;
			$chatId 		= !empty($params['chatId']) 	? sanitize_text_field($params['chatId']) : 0;
			$chatType 		= !empty($params['chatType']) 	? intval($params['chatType']) 	: 0;
			...
			$chatId[0] 	= isset($chatId[0]) ? esc_sql($chatId[0]) : 0;
			$chatId[1] 	= isset($chatId[1]) ? esc_sql($chatId[1]) : 0;
			$verifyMember = false;
			if ($chatType == 1) {
				$receiverId = $chatId[0];
		...
		{
			$params     	= !empty($data->get_params()) 		? $data->get_params() 		: '';
			$loginedUser 	= !empty($data['userId']) 	? $data['userId'] 	: 0;
			$chatId 		= !empty($params['chatId']) 		? $params['chatId'] 		: '';
			$userType 		= isset($params['userType']) 		? intval($params['userType']) 		: 1;
			$text 			= !empty($params['text']) 			? sanitize_text_field($params['text']) 			: '';
			$userName 		= !empty($params['userName']) 		? sanitize_text_field($params['userName']) 		: '';
			$senderId 		= !empty($params['senderId']) 		? intval($params['senderId']) 		: 0;
			$chatType 		= isset($params['chatType']) 		? intval($params['chatType']) 		: '';
			...
			if (!empty($chatId)) {
				...
				$chatId 		= explode('_', $chatId);
				$chatId[0] 	= isset($chatId[0]) ? intval($chatId[0]) : 0;
				$chatId[1] 	= isset($chatId[1]) ? intval($chatId[1]) : 0;
				if ($chatType == 1 || $chatType == 3) {
					$receiverId = $chatId[0];
