...
if ( version_compare( PHP_VERSION, '8.1', '<' ) ) {
	add_action( 'admin_notices', 'wp_guppy_lite_minimum_php_version_notice' );
	return;
}
 ...
 */
function wp_guppy_lite_minimum_php_version_notice() {
	$message = sprintf(
	/* translators: 1: Plugin name 2: WP Meet Press */
		esc_html__( '%1$s requires %2$s. Please upgrade your PHP version to activate the plugin.', 'wpguppy-lite' ),
		'<strong>' . esc_html__( 'WP Guppy Lite', 'wpguppy-lite' ) . '</strong>',
		'<strong>' . esc_html__( 'PHP version 8.1 or higher', 'wpguppy-lite' ) . '</strong>'
	);
